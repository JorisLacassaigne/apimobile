 ...
 * @param {string} codev - ID du vendeur
 ...
 */
const processOrder = async (codev, codec, dateCommande, totalHT, lignesCommande) => {
    ...
    try {
        ...
        const [orderResult] = await connection.query(
            'INSERT INTO orders (codev, codec, date_commande, total_ht) VALUES (?, ?, ?, ?)',
            [codev, codec, dateCommande, totalHT]
        );

        const numero = orderResult.insertId;

        // Insérer les lignes de commande
        for (const ligne of lignesCommande) {
            await connection.query(
                'INSERT INTO order_lines (numero_ligne, reference, quantite_demandee) VALUES (?, ?, ?)',
                [numero, ligne.reference, ligne.quantite_demandee]
            );
        }
